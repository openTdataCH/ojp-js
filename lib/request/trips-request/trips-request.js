...
export class TripRequest extends OJPBaseRequest {
    constructor(stageConfig, language, xmlConfig, requestorRef, fromTripLocation, toTripLocation, departureDate = new Date(), tripRequestBoardingType = 'Dep', numberOfResults = null, numberOfResultsBefore = null, numberOfResultsAfter = null, publicTransportModes = []) {
    ...
    static Empty(xmlConfig, requestorRef) {
    ...
    static initWithRequestMock(mockText, xmlConfig, requestorRef) {
        const request = TripRequest.Empty(xmlConfig, requestorRef);
        request.mockRequestXML = mockText;
        request.requestInfo.requestXML = mockText;
        return request;
    }
    static initWithStopRefs(stageConfig, language, xmlConfig, requestorRef, fromStopRef, toStopRef, departureDate = new Date(), tripRequestBoardingType = 'Dep') {
    ...
    static initWithLocationsAndDate(stageConfig, language, xmlConfig, requestorRef, fromLocation, toLocation, departureDate, tripRequestBoardingType = 'Dep') {
        const fromTripLocationPoint = new TripLocationPoint(fromLocation);
        const toTripLocationPoint = new TripLocationPoint(toLocation);
        const request = new TripRequest(stageConfig, language, xmlConfig, requestorRef, fromTripLocationPoint, toTripLocationPoint, departureDate, tripRequestBoardingType);
        return request;
    ...
    static initWithTripLocationsAndDate(stageConfig, language, xmlConfig, requestorRef, fromTripLocation, toTripLocation, departureDate, tripRequestBoardingType = 'Dep', includeLegProjection = false, modeType = 'monomodal', transportMode = 'public_transport', viaTripLocations = [], numberOfResults = null, numberOfResultsBefore = null, numberOfResultsAfter = null, publicTransportModes = []) {
        ...
        const request = new TripRequest(stageConfig, language, xmlConfig, requestorRef, fromTripLocation, toTripLocation, departureDate, tripRequestBoardingType, numberOfResults, numberOfResultsBefore, numberOfResultsAfter, publicTransportModes);
